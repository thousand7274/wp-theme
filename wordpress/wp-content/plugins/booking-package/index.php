...
*/
	...
	class BOOKING_PACKAGE {
		...
		public $javascriptEditor = 0;
		
		public $taxForDay = 1;
		
		public $siteNetwork = 0;
		
		public $javascriptForUser = 0;
		...
		public function login_errors($error) {
			...
			if (isset($_POST['redirect_to']) && isset($_POST['puginName']) && $_POST['puginName'] == 'booking-package') {
		...
		public function admin_bar_menu($wp_admin_bar){
			...
			if(current_user_can($roleName) === false && current_user_can('subscriber') === false){
		...
		public function bookingPageForVisitors($atts){
			...
			$formData = $setting->getForm($accountKey);
			...
			$countServices = 0;
			foreach ($courseList as $key => $service) {
			...
			$schedule->insertAccountSchedule(date('m'), date('d'), date('Y'));
			...
			foreach ($postPages as $key => &$value) {
			...
			if (!is_null($calendarAccount['redirectPage'])) {
			...
			**/
			...
			if (count($paymentMethod) == 1 && strlen($paymentMethod[0]) == 0) {
					...
					<div class="edit_modal_content">
						...
						<div id="media_router" class="media_frame_router">
							<div class="reservation_table_row">
								<div id="reservation_users" class="media_menu_item active"><?php _e("List of visitors", $pluginName); ?></div>
		...
		public function members_page(){
			...
			if(intval($memberSetting['accept_subscribers_as_users']['value']) == 0){
				...
				<div id="member_list">
					...
					<div class="page_action_panel">
						<select id="swich_authority" class="select_limit<?php echo $swich_authority_by_hidden; ?>">
							...
							<option value="subscriber"><?php echo __("Subscriber", $pluginName); ?></option>
						</select>
		...
		public function schedule_page(){
			...
			?>
				...
				<div id="select_package" class="wrap">
		...
		public function setting_page(){
			...
			if ($this->javascriptForUser == 1) {
				...
				$localize_script['javascriptForUser'] = $this->javascriptForUser;
					...
					<div id="tabFrame">
						...
						<div id="contentPanel" class="content">
							...
							<div id="javascriptPanel" class="hidden_panel">
								...
								<div style="padding-bottom: 1em;"><?php _e("Change the front-end page design by defining JavaScript.", $pluginName); ?></div>
						...
						<div id="media_frame_content" class="media_left_zero content_top_48">
							...
							<div class="mail_message_area_left">
								...
								<?php 
									...
									foreach ($emailMessageList as $key => $mailMessage) {
		...
		public function wp_ajax_booking_package_for_public(){
			...
			if (isset($_POST['nonce']) && check_ajax_referer($this->action_public."_ajax", 'nonce', false)) {
				...
				if ($_POST['mode'] == 'cancelBookingData' && isset($_POST['key']) && isset($_POST['token'])) {
					...
					$response = $schedule->cancelBookingData(intval($_POST['key']), $_POST['token']);
				...
				if ($_POST['mode'] == 'cancelUserBooking') {
					...
					if (intval($user['status']) == 1 && intval($user['user']['current_member_id']) == intval($_POST['user_id'])) {
						...
						$response = $schedule->updateStatus();
						$response['reload'] = 0;
		...
		public function choeseMode(){
			...
			if($_POST['mode'] == 'getForm'){
				...
				$response = $setting->getForm($_POST['accountKey']);
			...
			if($_POST['mode'] == $this->prefix.'getReservationData'){
				...
				if(isset($_POST['accountKey'])){
					...
					$response['formData'] = $setting->getForm($_POST['accountKey']);
			...
			if($_POST['mode'] == 'deleteBookingData'){
				...
				$response = $schedule->deleteBookingData($_POST['key'], false, true, $_POST['sendEmail']);
				$response['guestsList'] = $setting->getGuestsList($_POST['accountKey'], true);
				$response['account'] = $schedule->getCalendarAccount($_POST['accountKey']);
			...
			if($_POST['mode'] == 'updateBooking'){
				...
				$response = $schedule->updateBooking();
				$response['guestsList'] = $setting->getGuestsList($_POST['accountKey'], true);
				$response['account'] = $schedule->getCalendarAccount($_POST['accountKey']);
		...
		public function upgradeButton($isExtensionsValid = false){
			...
			$upgradeDetail = array("extension_url" => BOOKING_PACKAGE_EXTENSION_URL, "timeZone" => $timezone, "local" => get_locale(), "site" => get_site_url(), "stripe_public_key" => BOOKING_PACKAGE_STRIPE_PUBLIC_KEY, "locale" => $locale);
			...
			$posts = array("extension_url", "local", "timeZone", "site", "pluginUrl");
		...
		public function api(){
			...
			if(isset($_POST['mode'])){
				...
				if(isset($_POST['access_status']) && $_POST['access_status'] == 'public'){
					
					if($_POST['mode'] == 'getNonce' && isset($_POST['accountKey'])){
						...
						for($i = 0; $i < count($guestsList); $i++){
							...
							$decode = json_decode($guestsList[$i]['json'], true);
							$guestsList[$i]['json'] = $decode;
						...
						$localize_script = array(
							...
							'prefix' => $this->prefix,
							'courseBool' => $courseBool, 
							'courseName' => $calendarAccount["courseTitle"], 
							'year' => date('Y'), 
							'month' => date('m'), 
							'day' => date('d'), 
							'courseList' => $courseList, 
							...
							'is_mobile' => $this->is_mobile,
							'style' => $this->getStyle($list),
							'javascriptSyntaxErrorNotification' => 1,
							'javascriptFileslist' => $javascriptFileslist,
						...
						print json_encode($localize_script);
						
					}else{
						
						$this->wp_ajax_booking_package_for_public();
		...
		**/
		...
		public function getDictionary($mode, $pluginName){
			...
			if($mode == "adomin"){
				...
				$dictionary['Cancellation of booking by visitor and user'] = __('Cancellation of booking by visitor and user', $pluginName);
