    ...
    class booking_package_schedule {
        ...
        public function get_user($userId = null, $statusCheck = true){
        	...
        	if(is_null($userId)){
				...
				if(is_array(array_values($value))){
			...
			if($bool === true){
				...
				if(!is_null($row)){
					...
					for($i = 0; $i < count($form); $i++){
						...
						$type = $form[$i]->type;
						$formId = $form[$i]->id;
						$array = array("id" => $formId, "value" => $form[$i]->value);
			...
			**/
			...
			foreach($calendarList as $key => $value){
				...
				for($i = $value['startDay']; $i <= $value['lastDay']; $i++){
					...
					$dayArray = array('year' => $value['year'], 'month' => $value['month'], 'day' => $i, 'count' => null, 'accountKey' => $accountKey, 'status' => 0);
					$list[$key] = $dayArray;
        ...
        public function getCalendarAccount($accountKey = 1){
        ...
        public function createCloneCalendar() {
        	...
        	foreach ($targetList as $key => $table) {
        ...
        public function updateCalendarAccount(){
        	...
        	$bool = $wpdb->update(
				...
				array(
					...
					'courseTitle' => sanitize_text_field($_POST['courseTitle']),
					'courseBool' => intval($_POST['courseBool']),
					'googleCalendarID' => sanitize_text_field($_POST['googleCalendarID']),
					'uploadDate' => date('U'),
					'cost' => intval($_POST['cost']),
					'numberOfRoomsAvailable' => intval($_POST['numberOfRoomsAvailable']),
					'numberOfPeopleInRoom' => intval($_POST['numberOfPeopleInRoom']),
        ...
        public function updateAccountTemplateSchedule(){
            ...
            **/
            ...
            for($i = 0; $i < $_POST['timeCount']; $i++){
				...
				$unixTime = mktime(intval($_POST['hour'.$i]), intval($_POST['min'.$i]), 0, intval($_POST['month']), intval($_POST['day0']), intval($_POST['year']));
				$valueArray = array($accountKey, intval($_POST['hour'.$i]), intval($_POST['min'.$i]), intval($_POST['weekKey']));
				$table_name = $wpdb->prefix."booking_package_templateSchedule";
				$sql = $wpdb->prepare(
					"SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND `hour` = %d AND `min` = %d AND `weekKey` = %d;", 
					$valueArray
			...
			$month = date('m');
			$day = date('d');
			$this->insertAccountSchedule($month, $day, $year);
			#$templateSchedules = $this->getTemplateSchedule(intval($_POST['weekKey']));
        ...
        public function insertAccountSchedule($month, $day, $year){
    					...
    					if($calendarAccount['type'] == 'day'){
    						...
    						$wpdb->insert(
    							...
    							array(
    								...
    								'min' => 0, 
    								'title' => '', 
    								'cost' => intval($calendarAccount['cost']), 
    								'capacity' => intval($capacity), 
    								'remainder' => intval($capacity), 
    								'stop' => 'false', 
    								'holiday' => 'false', 
			...
			if($ical === false){
				...
				foreach($calendarList as $key => $value){
					...
					for($i = $value['startDay']; $i <= $value['lastDay']; $i++){
    					...
    					$arrayValue = array('number' => $number, 'year' => $value['year'], 'month' => $value['month'], 'day' => $i, 'week' => $week, 'select' => 'false');
    					$number++;
						...
						$sql = $wpdb->prepare(
							...
							array(intval($accountKey), intval($value['year']), intval($value['month']), intval($i))
						);
						$rows = $wpdb->get_results($sql, ARRAY_A);
					...
					$sql = $wpdb->prepare(
						...
						array(intval($accountKey), intval($value['year']), intval($value['month']), intval($day))
					);
    	...
    	public function serachCourse($accountKey, $key = false, $time = false){
    		...
    		if($key !== false){
    			...
    			$sql = $wpdb->prepare(
    				"SELECT `key`,`name`,`time`,`cost` FROM `".$table_name."` WHERE `accountKey` = %d AND `key` = %d LIMIT 0, 1;", 
    		...
    		if($time != false){
    			...
    			$sql = $wpdb->prepare(
    				"SELECT `key`,`name`,`time`,`cost` FROM `".$table_name."` WHERE `accountKey` = %d AND `time` = %d LIMIT 0, 1;", 
    	...
    	public function createAccommodationDetails($calendarAccount, $json, $sql_start_unixTime, $applicantCount, $type, $accommodationDetails = null){
    		...
    		if(is_null($accommodationDetails)){
    			...
    			$accommodationDetails = array("scheduleList" => array(), "guestsList" => array(), 'type' => $calendarAccount['type'], 'taxes' => array());
    		...
    		if (count($jsonList['guestsList']) != 0) {
    			
            	
            	$additionalFee = 0;
            	$table_name = $wpdb->prefix."booking_package_guests";
            	$guestsList = $jsonList['guestsList'];
            	foreach($guestsList as $key => $value){
		            ...
		            if ($tax['target'] == 'room') {
		                ...
		                if ($tax['scope'] == 'day') {
		                    ...
		                    if ($tax['method'] == 'addition') {
		                        ...
		                        $taxValue = $accommodationDetails['nights'] * $value;
		                    ...
		                    if ($tax['method'] == 'addition') {
		                        ...
		                        $taxValue = ($accommodationDetails['adult'] + $accommodationDetails['children']) * $value;
		                        
		                    } else if ($tax['method'] == 'multiplication') {
		                        
		                        $taxValue =  ($value / 100) * ($accommodationDetails['adult'] + $accommodationDetails['children']);
		                ...
		                if ($tax['scope'] == 'day') {
		                    ...
		                    if ($tax['method'] == 'addition') {
		                        ...
		                        $taxValue = ($accommodationDetails['nights'] * ($accommodationDetails['adult'] + $accommodationDetails['children'])) * $value;
    	...
    	public function sendBooking($administrator = false) {
    		...
    		$form = $this->getUserValues($accountKey);
			...
			if (is_null($row)) {
    			...
    			if (intval($row['unixTimeDeadline']) < $timestamp) {
    			...
    			$scheduleUnixTime = intval($_POST['unixTime']);
				...
				if($row['unixTime'] == $scheduleUnixTime){
						...
						if(isset($_POST['courseKey']) || isset($_POST['selectedCourseList'])){
							...
							if ($this->hasMultipleServices == 1) {
								...
								foreach ($services as $key => $service) {
									...
									$row = $this->serachCourse($accountKey ,$service['key']);
									if ($row['status'] == 'error') {
										
										#$wpdb->query('ROLLBACK');
										return array('status' => 'error', 'error' => '9020');
								...
								#return array("status" => "error", "servicesDetails" => $servicesDetails, "valueArray" => $valueArray);
								
							} else {
								
								$row = $this->serachCourse($accountKey ,$_POST['courseKey']);
								if ($row['status'] == 'error') {
									...
									$courseName .= $row['name']."\n";
									$courseCost += intval($row['cost']);
									$courseTime += $row['time'];
									$totalCost += intval($row['cost']) * $applicantCount;
									$service = array("name" => $row['name'], "price" => $row['cost'], "time" => $row['time']);
									
									$selectedOptionsObject = $this->getSelectedOptions($_POST['selectedOptions'], $applicantCount);
									$selectedOptions = $selectedOptionsObject['object'];
									$totalCost += $selectedOptionsObject['cost'];
									
									$sql_max_unixTime = $sql_start_unixTime + ($courseTime * 60) + ($maintenanceTime * 60) + ($selectedOptionsObject['time'] * 60);
									$table_name = $wpdb->prefix."booking_package_schedule";
									$account_sql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND (`unixTime` >= %d AND `unixTime` < %d) ORDER BY `unixTime` ASC ;";
									#$valueArray = array(intval($accountKey), intval($sql_start_unixTime), intval($sql_max_unixTime));
									if (isset($preparation['position']) && $preparation['position'] == 'before_after' || $preparation['position'] == 'before') {
							...
							**/
							...
							#$valueArray = array(intval($accountKey), intval($_POST['timeKey']));
							$account_sql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND (`unixTime` >= %d AND `unixTime` <= %d) ORDER BY `unixTime` ASC ;";
							if (isset($preparation['position']) && $preparation['position'] == 'before_after' || $preparation['position'] == 'before') {
					...
					$privateResponse = $this->insertPrivateData($sendDate, $_POST['permission'], $status, $_POST['timeKey'], $scheduleUnixTime, $scheduleTitle, $scheduleCost, $services, $form, $currency, $_POST['payType'], $cardToken, $accountKey, $permalink, $preparation, $taxes, $applicantCount);
    		...
    		if(intval($_POST['sendEmail']) == 1){
    			...
    			$email = $this->createEmailMessage($accountKey, array('mail_new_admin'), $form, $accommodationDetails, $selectedOptions, $lastID, $scheduleUnixTime, $cancellationUri, $currency, $services, $payName, $payId, $scheduleTitle);
    	...
    	public function insertPrivateData($sendDate, $permission, $status, $timeKey, $scheduleUnixTime, $scheduleTitle, $scheduleCost, $services, $form, $currency, $payType, $cardToken, $accountKey, $permalink, $preparation, $taxes, $applicantCount = 1){
    	...
    	public function cancelBookingData($deleteKey, $token) {
    		...
    		if ($verifyCancellation['cancel'] === true) {
    			...
    			$deleteResponse = $this->deleteBookingData($deleteKey, false, false, 0);
    			if ($deleteResponse['status'] == 'success') {
	    			...
	    			$bool = $wpdb->update(
	    				$table_name,
						array('status' => sanitize_text_field("canceled")),
						array('key' => intval($deleteKey)),
						array('%s'),
						array('%d')
					);
    				
    				$accommodationDetails = array();
    				if (count($myBookingDetails['accommodationDetailsList']) > 0) {
    					
    					$accommodationDetails = json_decode($myBookingDetails['accommodationDetails'], true);
    					
    				}
    				
    				$service = null;
    				$service = array("name" => $myBookingDetails['courseName'], "price" => $myBookingDetails['courseCost'], "time" => $myBookingDetails["courseTime"]);
    				
    				$servicesDetails = $this->getSelectedServices(json_decode($myBookingDetails['options'], true), "options", $applicantCount);
					$services = $servicesDetails['object'];
    				
    				$email = $this->createEmailMessage(
    					$myBookingDetails['accountKey'], 
    					array('mail_canceled_by_visitor_user'), 
    					json_decode($myBookingDetails['praivateData']), 
    					$accommodationDetails, 
    					json_decode($myBookingDetails['options'], true), 
    					$deleteKey, 
    					intval($myBookingDetails['scheduleUnixTime']), 
    					null, 
    					$myBookingDetails['currency'], 
    					$services, 
    					$myBookingDetails['payId'],
    					$myBookingDetails['scheduleTitle']
    				);
    	...
    	public function deleteBookingData($deleteKey = false, $sendGC = true, $deleteVisitorDetails = true, $sendEmail = 1){
            	...
            	if (is_null($row) === false) {
					...
					if ($status != 'canceled') {
						...
						if ($calendarAccount['type'] == 'hotel') {
							...
							$account_sql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND (`unixTime` >= %d AND `unixTime` <= %d) ORDER BY `unixTime` ASC ;";
							$valueArray = array(intval($accountKey), intval($unixTimeStart), intval($unixTimeEnd));
	            			...
	            			if (strlen($row['courseKey']) != 0 || $hasMultipleServices == 1) {
	            				...
	            				#$valueArray = array(intval($accountKey), intval($row['scheduleKey']));
					...
					if (intval($sendEmail) == 1) {
						...
						$email = $this->createEmailMessage($accountKey, array('mail_deleted'), $form, $accommodationDetails, $options, $deleteKey, intval($unixTimeStart), null, $currency, $services, $payName, $payId, $scheduleTitle);
            	...
            	if(isset($_POST['update_booking_date']) || isset($_POST['update_booking_course'])){
    				...
    				if($calendarAccount['type'] == 'hotel'){
            			...
            			if (count($services) > 0) {
            				...
            				$deleteValueArray = array(intval($accountKey), intval($unixTimeStart), intval($unixTimeEnd));
            				...
            				if (isset($preparation['time']) && intval(isset($preparation['time'])) > 0) {
            					...
            					$deleteValueArray = array(intval($accountKey), intval($scheduleKey));
    					...
    					/**
    					if(isset($_POST['update_booking_course'])){
    						
    						$rowCourse = $this->serachCourse($accountKey, $_POST['update_booking_course']);
							if($rowCourse['status'] == 'error'){
            					...
            					$courseKey = $rowCourse['key'];
    							$courseName = $rowCourse['name'];
    							$courseTime = $rowCourse['time'];
    							$courseCost = $rowCourse['cost'];
    					...
    					**/
    					...
    					if (count($selectedServices) > 0) {
            				...
            				$updateValueArray = array(intval($accountKey), intval($unixTimeStart), intval($unixTimeEnd));
            				...
            				if (isset($preparation['time']) && intval(isset($preparation['time'])) > 0) {
            					...
            					$updateValueArray = array(intval($accountKey), intval($scheduleKey));
            		...
            		#$updateSchedule = $this->updateRemainderSeart("increase", $updateSql, $updateValueArray, $applicantCount);
    	...
    	public function updateStatus(){
    		...
    		if (isset($_POST['newStatus'])) {
    			...
    			$status = strtolower($_POST['newStatus']);
    		...
    		if ($status == 'canceled') {
    			...
    			$_POST['sendEmail'] = 0;
    			$this->deleteBookingData($_POST['key'], false, false, 0);
    		...
    		$table_name = $wpdb->prefix."booking_package_userPraivateData";
    		$sql = "SELECT * FROM `".$table_name."` WHERE `key` = %d;";
    		$sql = $wpdb->prepare("SELECT * FROM `".$table_name."` WHERE `key` = %d;", array(intval($_POST['key'])));
    		$row = $wpdb->get_row($sql, ARRAY_A);
            if(is_null($row) === false){
    		...
    		if(intval($sendEmail) == 1){
    			...
    			$email = $this->createEmailMessage($accountKey, array($email_id), $form, $accommodationDetails, $options, intval($_POST['key']), intval($unixTimeStart), $cancellationUri, $currency, $services, $payName, $payId, $scheduleTitle);
    			#$ressponse['email'] = $email;
    			#return $email;
    		}
    	...
    	public function changeBookingTime($mode, $updateKey, $updateScheduleKey, $status, $applicantCount, $newTimeStart, $newTimeEnd, $oldTimeStart, $oldTimeEnd, $accommodationDetails, $accountKey = 1){
    		...
    		$table_name = $wpdb->prefix."booking_package_schedule";
    		$updateSql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND (`unixTime` >= %d AND `unixTime` < %d) ORDER BY `unixTime` ASC ;";
			$updateValue = array(intval($accountKey), intval($newTimeStart), intval($newTimeEnd));
			if($newTimeStart == $newTimeEnd){
				
				$updateSql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND `unixTime` = %d;";
				$updateValue = array(intval($accountKey), intval($newTimeStart));
			...
			if($changeBool === true){
				...
				if($mode == 'update'){
					...
					$checkOut = 0;
					$deleteSql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND (`unixTime` >= %d AND `unixTime` < %d) ORDER BY `unixTime` ASC ;";
					$deleteValue = array(intval($accountKey), intval($oldTimeStart), intval($oldTimeEnd));
					if($oldTimeStart == $oldTimeEnd){
						
						$deleteSql = "SELECT * FROM `".$table_name."` WHERE `accountKey` = %d AND `key` = %d;";
						$deleteValue = array(intval($accountKey), intval($updateScheduleKey));
					...
					#$this->updateRemainderSeart("increase", $updateSql, $updateValue, $applicantCount);
    	...
    	#public function updateRemainderSeart($mode, $sql, $valueArray, $applicantCount = 1){
    	...
    	public function getUserValues($accountKey){
    		...
    		for($i = 0; $i < count($form); $i++){
    			...
    			if (isset($_POST['form'.$i])) {
    				...
    				#if ($form[$i]->required == 'true' && strlen($_POST['form'.$i]) == 0) {
    				if ($form[$i]->required == 'true' && strlen(preg_replace("/( |　)/", "", $_POST['form'.$i])) == 0) {
    					...
    					if ($form[$i]->isEmail == 'true' && strlen($_POST['form'.$i]) != 0 && is_email($_POST['form'.$i]) === false) {
    						...
    						return array('status' => 'error', "message" => __('The format of the email address is incorrect.', $this->pluginName), 'form' => $form[$i]);
    	...
    	public function bookingDetailsForHotel($accommodationDetails, $currency, $mode = 'array'){
			...
			$detailsList = array(__('Total length of stay', $this->pluginName).": ".$accommodationDetails['nights']." ".$nights." ".$this->formatCost($accommodationDetails['accommodationFee'], $currency)."");
			$objectList = array(
			...
			foreach ($scheduleDetails as $key => $value) {
				...
				if (intval($value['cost']) > 0) {
					...
					$details .= $this->formatCost($value['cost'], $currency);
			...
			if (intval($accommodationDetails['additionalFee']) > 0) {
				
				$people .= "".$this->formatCost($accommodationDetails['additionalFee'], $currency)."";
    	...
    	public function createEmailMessage($accountKey, $email_id, $form, $accommodationDetails, $options, $bookingID, $unixTime, $cancellationUri, $currency = 'usd', $services = null, $payName = null, $payId = null, $scheduleTitle = null){
			...
			foreach ($sendEamilList as $emailKey => $target) {
				...
				foreach ($emailData as $emailDataKey => &$emailDataValue) {
	    		...
	    		if (preg_match('/(\[paymentMethod\])/', $emailBody, $matches)) {
	    			...
	    			$emailBody = preg_replace('/(\[paymentMethod\])/', $payName, $emailBody);
